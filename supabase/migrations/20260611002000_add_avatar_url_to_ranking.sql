-- 리더보드에 구글 프로필 이미지를 보여주기 위해 profiles에 avatar_url을 저장한다
...
as $$
  ...
  totals as (
    select
      ...
      -- 앱 전체 표기와 동일하게 cache 포함 총합으로 집계한다
      sum(d.input_tokens + d.output_tokens + d.cache_tokens) as total_tokens,
      sum(d.input_tokens + d.output_tokens + d.cache_tokens)
        filter (where d.provider = 'claude') as claude_tokens,
      sum(d.input_tokens + d.output_tokens + d.cache_tokens)
        filter (where d.provider = 'codex') as codex_tokens
  ...
  ranked as (
    select
      ...
      t.cache_tokens,
      t.total_tokens,
      coalesce(t.claude_tokens, 0) as claude_tokens,
      coalesce(t.codex_tokens, 0) as codex_tokens,
      t.user_id
    from totals t
